...
<div class="mt-30vh h-full max-w-md w-3/4 xl:max-w-lg">
  <div in:scale|global={{ start: 0.9, easing: cubicOut }} out:scale|global={{ start: 0.95, easing: cubicIn }} use:draggable={{ applyUserSelectHack: false, cancel: ref, bounds: { left: 20, top: 20, right: 20, bottom: 20 } }} class="flex flex-col cursor-grab rounded-lg bg-neutral-8/60 p-4 ring-1.3 ring-white/20 backdrop-blur-md xl:max-w-3xl" class:gap-4={html}>
    ...
    <div class="cursor-auto [&>article]:(<sm:text-xs xl:text-base)" bind:this={ref}>
      <Markdown {html} />
    </div>
  </div>
</div>
